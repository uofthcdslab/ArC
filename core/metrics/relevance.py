...
class SoSMetric(BaseMetric):
    ...
    def compute(self, sample_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        Args:
            sample_data: Contains justify_reasons_confidences and justify_reasons_sims_input

        Returns:
            Dictionary with SoS scores for each reason
        """
        confidences = sample_data.get('justify_reasons_confidences', [])
        sims_input = sample_data.get('justify_reasons_sims_input', [])
...
class DiSMetric(BaseMetric):
    ...
    def compute(self, sample_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        Args:
            sample_data: Contains justify_reasons, justify_reasons_confidences,
                        and justify_reasons_sims_reasons

        Returns:
            Dictionary with DiS_avg score
        """
        reasons = sample_data.get('justify_reasons', [])
        confidences = sample_data.get('justify_reasons_confidences', [])
        sims_reasons = sample_data.get('justify_reasons_sims_reasons', [])
